...
class App:
    ...
    def __init__(self, title: str = 'botbowl', fps: int = 60,
                 ...
                 auto_screenshot: str = None, auto_screenshot_frames: int = 3):
        pygame.init()
        pygame.font.init()
        ...
        self._auto_screenshot_path = auto_screenshot
        self._auto_screenshot_frames = auto_screenshot_frames
        self._frame_count = 0
    ...
    def pop_to_root(self):
        """Pop all screens except the first (root) screen."""
    ...
    def run(self):
        ...
        while self._running:
            ...
            # Auto-screenshot: save after N frames then quit
            if (self._auto_screenshot_path and
                    self._frame_count >= self._auto_screenshot_frames):
                os.makedirs(os.path.dirname(os.path.abspath(
                    self._auto_screenshot_path)), exist_ok=True)
                pygame.image.save(surface, self._auto_screenshot_path)
                print(f'Auto-screenshot: {self._auto_screenshot_path}')
                self._running = False
                break
